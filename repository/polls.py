...
class PollRepository:
    ...
    @staticmethod
    async def fetch_poll_or_none(poll_id: int, session: AsyncSession) -> Union[Poll, JSONResponse]:
    ...
    @staticmethod
    async def poll_is_active(poll_id: int, session: AsyncSession) -> Union[bool, JSONResponse]:
    ...
    @staticmethod
    async def get_questions_by_poll_id(poll_id: int, session: AsyncSession) -> dict[str: List[str]]:
        res = await session.scalars(select(Question).where(Question.poll_id == poll_id))
        questions = res.all()
        if not questions:
            return {'questions': []}
        return {'questions': [quest.title for quest in res]}



    @staticmethod
    async def create_poll(user_id: int, create_poll: PollInFirst, session: AsyncSession) -> dict[str:bool]:
    ...
    @staticmethod
    async def update_poll(id_poll: int, upd_poll: PollInChange, session: AsyncSession) -> dict[str:bool]:
    ...
    @staticmethod
    async def delete_poll(id_poll: int, session: AsyncSession) -> dict[str:bool]:
        poll_in_db = await PollRepository.fetch_poll_or_none(id_poll, session)
        if poll_in_db is None:
            raise HTTPException(404, detail='Poll not found')
    ...
    @staticmethod
    async def add_question_in_poll(id_poll: int, question: QuestionIn, session: AsyncSession) -> dict[str:bool]:
    ...
    @staticmethod
    async def delete_question_in_poll(id_question: int, session: AsyncSession) -> dict[str:bool]:
